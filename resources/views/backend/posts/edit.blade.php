					...
					<form role="form" method="post" action="{{ route('backend.post.update',$post->id) }}" enctype="multipart/form-data">
						...
						<div class="card-body">
							<div class="form-group">
								...
								<input type="text" name="name" class="form-control" id="" placeholder="Điền tiêu đề bài viết" value="{{$post->name}}">
								@error('name')
							...
							<div class="form-group">
								...
								<input type="text" name="description" class="form-control" id="" placeholder="Điền mô tả bài viết" value="{{$post->description}}">
								@error('name')
								<p style="color: red;">{{$message}}</p>
								@enderror
							</div>
							...
							<div class="form-group">
								...
								style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;" value="">{{$post->content}}}</textarea>
								@error('content')
								<p style="color: red;">{{$message}}</p>
								@enderror
							</div>
						...
						<div class="card-footer">
							<a href="{{ route('backend.post.index') }}" class="btn btn-default">Huỷ bỏ</a>
